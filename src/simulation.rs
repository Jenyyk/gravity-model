...
impl Simulation {
    ...
    pub fn calculate_stable_orbit_velocities(&self) -> Result<(float2, float2), String> {
        ...
        if bodies.len() != 2 {
            return Err(String::from("Function only available for 2-body systems"));
        }

        let distance1: float2 = bodies[1].position - bodies[0].position;
        let vel1: f64 = ((GRAVITATIONAL_CONSTANT * bodies[1].mass)
            / (distance1.abs() * (1_f64 + (bodies[0].mass / bodies[1].mass))))
            .sqrt();
        let vel1vec: float2 = distance1.normalize().perpendicular() * vel1;

        let distance2: float2 = bodies[0].position - bodies[1].position;
        let vel2: f64 = ((GRAVITATIONAL_CONSTANT * bodies[0].mass)
            / (distance2.abs() * (1_f64 + (bodies[1].mass / bodies[0].mass))))
            .sqrt();
